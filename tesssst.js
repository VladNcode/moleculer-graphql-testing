...
module.exports = {
	...
	actions: {
		...
		resolveToken: {
			...
			async handler(ctx) {
				...
				if (!decoded.id) throw new MoleculerClientError('Invalid token', 401, 'INVALID_TOKEN');
		...
		register: {
			...
			async handler(ctx) {
				if (!this.config['accounts.signup.enabled'])
					throw new MoleculerClientError('Sign up is not available.', 400, 'SIGNUP_DISABLED');
				...
				if (found)
					throw new MoleculerClientError('Email has already been registered.', 400, 'EMAIL_EXISTS');

				// Verify username
				if (this.config['accounts.username.enabled']) {
					if (!entity.username) {
						throw new MoleculerClientError("Username can't be empty.", 400, 'USERNAME_EMPTY');
					}
				...
				if (entity.password) {
					...
					throw new MoleculerClientError("Password can't be empty.", 400, 'PASSWORD_EMPTY');
				}
	...
	methods: {
		...
		checkUser(user, opts = {}) {
			if (!user) {
				throw new MoleculerClientError('Account is not registered.', 400, 'ACCOUNT_NOT_FOUND');
			}
		...
		async sendMagicLink(ctx, user) {
			const token = await this.generateToken(C.TOKEN_TYPE_PASSWORDLESS, user.id, TOKEN_EXPIRATION);
